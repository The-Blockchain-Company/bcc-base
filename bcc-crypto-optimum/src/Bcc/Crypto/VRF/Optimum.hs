...
-- implementation in https://github.com/The-Blockchain-Company/libsodium
module Bcc.Crypto.VRF.Optimum
  (
  -- * VRFAlgorithm API
    OptimumVRF
...
import Bcc.Crypto.Seed (getBytesFromSeedT)
import Bcc.Crypto.Util (SignableRepresentation(..))

import Bcc.Prelude (NFData (..))
...
-- for the 'OptimumVRF' algorithm. Unlike the 'genKeyPairVRF' method, which uses
-- a 'ByteString'-based 'Bcc.Crypto.Seed.Seed', this seed generation method
...
instance VRFAlgorithm OptimumVRF where
  newtype VerKeyVRF OptimumVRF = VerKeyOptimumVRF VerKey
    deriving stock   (Show, Eq, Generic)
    deriving newtype (ToCBOR, FromCBOR)
    deriving NoThunks via OnlyCheckWhnfNamed "VerKeyVRF OptimumVRF" VerKey
    deriving newtype (NFData)

  newtype SignKeyVRF OptimumVRF = SignKeyOptimumVRF SignKey
    deriving stock   (Show, Eq, Generic)
    deriving newtype (ToCBOR, FromCBOR)
    deriving NoThunks via OnlyCheckWhnfNamed "SignKeyVRF OptimumVRF" SignKey
    deriving newtype (NFData)

  newtype CertVRF OptimumVRF = CertOptimumVRF Proof
    deriving stock   (Show, Eq, Generic)
    deriving newtype (ToCBOR, FromCBOR)
    deriving NoThunks via OnlyCheckWhnfNamed "CertKeyVRF OptimumVRF" Proof
    deriving newtype (NFData)

  type Signable OptimumVRF = SignableRepresentation

  algorithmNameVRF = const "OptimumVRF"

  deriveVerKeyVRF = coerce skToVerKey

  evalVRF = \_ msg (SignKeyOptimumVRF sk) ->
    ...
    in output `seq` proof `seq`
           (OutputVRF (outputBytes output), CertOptimumVRF proof)

  verifyVRF = \_ (VerKeyOptimumVRF pk) msg (_, CertOptimumVRF proof) ->
  ...
  genKeyPairVRF = \cryptoseed ->
    ...
    in sk `seq` pk `seq` (SignKeyOptimumVRF sk, VerKeyOptimumVRF pk)

  rawSerialiseVerKeyVRF (VerKeyOptimumVRF pk) = vkBytes pk
  rawSerialiseSignKeyVRF (SignKeyOptimumVRF sk) = skBytes sk
  rawSerialiseCertVRF (CertOptimumVRF proof) = proofBytes proof
  rawDeserialiseVerKeyVRF = fmap (VerKeyOptimumVRF . vkFromBytes) . assertLength verKeySizeVRF
  rawDeserialiseSignKeyVRF = fmap (SignKeyOptimumVRF . skFromBytes) . assertLength signKeySizeVRF
  rawDeserialiseCertVRF = fmap (CertOptimumVRF . proofFromBytes) . assertLength certSizeVRF

...
module Bcc.Crypto.Hash.SHA256
  ( SHA256
  )
where

import Control.Monad (unless)
import Bcc.Crypto.Libsodium.C (c_crypto_hash_sha256)
import Bcc.Foreign (SizedPtr(SizedPtr))
import Bcc.Crypto.Hash.Class (HashAlgorithm, SizeHash, hashAlgorithmName, digest)
